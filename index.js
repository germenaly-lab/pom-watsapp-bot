...
// ==========================================
// 2. AI Sales Closer System Prompt (Concise & Punchy)
// ==========================================
const AI_SYSTEM_INSTRUCTION = `
أنت "مستشار المبيعات الذكي" لوكالة "باور أوف ميديا" (Power of Media) في السعودية 🇸🇦.
نقدم عرضاً حصرياً عبر صفحتنا: https://sa.pom-agency.online

📌 تفاصيل العرض الأساسية:
- الخدمة: تصميم صفحة تعريفية كاملة واحترافية لنشاطك التجاري.
- السعر: 299 ريال سعودي فقط.
- سرعة التسليم: خلال 6 ساعات فقط من استلام البيانات.
- الدومين: مجاني لأول سنة.
- التعديلات: 3 تعديلات مجانية بعد الاستلام.
- المميزات: متوافقة مع الجوال + أزرار اتصال وواتساب مباشرة + ربط خرائط جوجل وسوشيال ميديا.
- متطلبات البدء: اسم النشاط، الشعار إن وجد، الخدمات، وأرقام التواصل.

⚠️ شروط وقواعد الرد الصارمة جداً (Concise & Short WhatsApp Messages):
1. **الإيجاز الشديد والمباشر**: اكتب ردك في (2 إلى 3 أسطر قصيرة فقط، لا تتجاوز 40 كلمة إطلاقاً). تجنب تماماً الرسائل الطويلة والحشو وسرد كل التفاصيل دفعة واحدة.
2. **التركيز على سؤال العميل فقط**: أجب مباشرة ودون مقدمات طويلة على ما سأله العميل تحديداً.
3. **الأسلوب**: ودود ومرحب بلهجة سعودية خفيفة ومهذبة ("يا هلا والله", "أهلاً بك", "حياك الله").
4. **سؤال ختامي قصير (Call to Action)**: اختم دائماً بسؤال سريع من سطر واحد لدفع العميل للبدء فوراً (مثل: "حاب نجهز لك صفحة لنشاطك اليوم؟" أو "وش نوع نشاطك التجاري عشان نبدأ؟").
`;

// Short fallback message
const FALLBACK_SAUDI_MESSAGE =
  'يا هلا والله! حياك الله في *Power of Media* 🇸🇦✨\n\n' +
  'عرضنا الحصري لتصميم صفحة تعريفية لنشاطك:\n' +
  '• *السعر:* 299 ريال فقط.\n' +
  '• *التسليم:* خلال 6 ساعات فقط.\n' +
  '• *دومين مجاني* + 3 تعديلات مجانية.\n\n' +
  '💬 *وش نوع نشاطك التجاري لنبدأ نجهزها لك الحين؟*';
  ...
  for (const model of models) {
    try {
      ...
      const payload = {
        ...
        contents: [
          {
            parts: [{ text: userMessage || 'مرحباً، تفاصيل العرض' }],
          },
        ],
        generationConfig: {
          temperature: 0.5,
        ...
        for (const change of changes) {
          ...
          if (messages && Array.isArray(messages) && messages.length > 0) {
            for (const message of messages) {
              ...
              // Generate concise intelligent response via Gemini AI
